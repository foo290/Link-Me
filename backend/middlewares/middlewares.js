...
function loggerMiddleware(req, res, next){
    print.debug(`Incoming request from user: ${req.user}, HOST: ${utils.getHost(req)}, IP: ${utils.getIP(req, true)}`)
    next()
}
...
function isAuthenticatedOrReadOnlyMiddleware(req, res, next){
    ...
    let _ = !loggedInUser? print.warning("User not logged in but fetching data using GET"): null
    next()
}
